...
const Uint32 DASH_COOLDOWN = 1000; // ms
...
int main(int argc, char* argv[]) {
    ...
    while (!quit) {
        ...
        if (!gameOver) {
            ...
            if (!dashing && keys[SDL_SCANCODE_LSHIFT] && canDash) {
                dashing = true;
            ...
            if (keys[SDL_SCANCODE_LEFT])  player.x -= moveSpeed;
            if (keys[SDL_SCANCODE_RIGHT]) player.x += moveSpeed;
            if (keys[SDL_SCANCODE_UP])    player.y -= moveSpeed;
            if (keys[SDL_SCANCODE_DOWN])  player.y += moveSpeed;
            ...
            // Fire bullet
            if (keys[SDL_SCANCODE_SPACE] && currentTime > lastBulletTime + fireCooldown) {
                GameObject bullet;
